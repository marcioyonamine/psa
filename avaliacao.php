			  ...
			  <form method="POST" action="?edital=<?php echo $_GET['edital']; ?>" class="form-horizontal" role="form" name="form1">
                ...
                <tr>
                  <td><?php echo $res[$i]['criterio']?></td>
					<td><input type="text" class="form-control nota" name="<?php echo $res[$i]['id']; ?>" value="<?php echo retornaNota($inscricao,$res[$i]['id'],$user->ID); ?>" max='3' min = '1'><?php echo $inscricao." - ".$res[$i]['id']." - ".$user->ID." - ".retornaNota($inscricao,$res[$i]['id'],$user->ID); ?></td>
				  </tr>
